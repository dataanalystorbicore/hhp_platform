...
def update_database(pos_report):
    ...
    df = df.merge(data_pos[["DISTRICT ID", "Latitude","Longitude"]], on="DISTRICT ID", how="left", suffixes=("", "_from_df2"))
    df["Latitude"] = df["Latitude"].fillna(df["Latitude_from_df2"])
    df["Longitude"] = df["Longitude"].fillna(df["Longitude_from_df2"])
    df = df.drop(columns=["Latitude_from_df2","Longitude_from_df2"])
    ...
    df = df[['Region_name','Wilaya','Commune','POSTCODE','Cluster', 'BBE (ORB)','APP CREDENTIALS','DISTRICT ID','POS Stat','Site ID','Grade','Pos Type','Name',
            'PropriétaireFirstname','PropriétaireLastname','PropriétairePhone', 'GérantFirstname','GérantLastname', 'GérantPhone', 
            'Address','Photo','CHANNEL','SUB-CHANNEL','DATA TYPE','TYPE DE PDV','SITUATION GEOGRAPHIQUE','Area','Latitude','Longitude','Coverage', 'Visit Day']]
